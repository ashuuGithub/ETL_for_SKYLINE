...
def fetch_data(sql_conn, schema, view_name, sql_columns):
    ...
    try:
        ...
        column_mappings = {
            'personGUID': 'CAST([personGUID] AS VARCHAR(36)) AS [personGUID]', # this is required for uniqueidentifier
   }
        column_str = ', '.join([column_mappings.get(col, f'[{col}]') for col in sql_columns])
        query = f"SELECT {column_str} FROM {schema}.{view_name}"
        df = pd.read_sql(query, sql_conn)
        logger.info(f"Fetched {len(df)} rows from {schema}.{view_name}")
        if len(df.columns) != len(sql_columns):
            logger.error(f"Column mismatch: Expected {len(sql_columns)} columns, got {len(df.columns)}")
            raise ValueError(f"Column mismatch: Expected {sql_columns}, got {df.columns.tolist()}")
        return df
    except Exception as e:
...
def main():
    ...
    view_name = 'individual'  # SQL Server view name
    table_name = 'individual_view'  # MySQL table name
    sql_columns = [
        'personID', 'stateID', 'studentNumber', 'staffNumber', 'staffStateID', 'geographicStaffStateID', 
        'personGUID', 'otherID', 'additionalID', 'edFiID', 'identityID', 'effectiveDate', 'lastName', 
        'firstName', 'middleName', 'suffix', 'alias', 'gender', 'birthdate', 'ssn', 'legalFirstName', 
        'legalLastName', 'legalMiddleName', 'legalSuffix', 'legalGender', 'raceEthnicity', 
        'homePrimaryLanguage', 'languageAlt', 'languageAlt2', 'districtID'
    ]
    mysql_columns = [
        'personID', 'stateID', 'studentNumber', 'staffNumber', 'staffStateID', 'geographicStaffStateID', 
        'personGUID', 'otherID', 'additionalID', 'edFiID', 'identityID', 'effectiveDate', 'lastName', 
